-- 23_trusted_places_family_locations_escalation.sql
-- Idempotent migration for:
--   1. trusted_places table
--   2. safe_windows new columns (trusted_place_id, completed_reason, severity, escalated_at)
--   3. family_member_locations table
...
CREATE TABLE IF NOT EXISTS public.trusted_places (
    ...
    name                      TEXT NOT NULL,
    label                     TEXT,   -- Home, Office, College, Hostel, Friend's House, Other
...
CREATE INDEX IF NOT EXISTS idx_trusted_places_user_active
    ON public.trusted_places(user_id, is_active);
...
ALTER TABLE public.safe_windows
    ...
    ADD COLUMN IF NOT EXISTS destination_radius_meters INTEGER DEFAULT 100,
    ADD COLUMN IF NOT EXISTS notify_guardians_on_arrival BOOLEAN DEFAULT TRUE,
    ADD COLUMN IF NOT EXISTS completed_reason       TEXT,  -- REACHED_TRUSTED_PLACE | MANUAL | TIMEOUT
    ADD COLUMN IF NOT EXISTS severity               TEXT NOT NULL DEFAULT 'NORMAL',  -- NORMAL | HIGH | CRITICAL | RESOLVED
...
CREATE INDEX IF NOT EXISTS idx_safe_windows_status_user
    ON public.safe_windows(status, user_id);

CREATE INDEX IF NOT EXISTS idx_safe_windows_escalated
    ON public.safe_windows(severity)
    WHERE severity IN ('HIGH', 'CRITICAL');


-- ============================================================
-- 3. family_member_locations
-- ============================================================
CREATE TABLE IF NOT EXISTS public.family_member_locations (
    ...
    status           TEXT NOT NULL DEFAULT 'SAFE',  -- SAFE | IN_SAFE_WINDOW | SOS_ACTIVE | CHECKIN_MISSED | OFFLINE
    source           TEXT,
...
CREATE INDEX IF NOT EXISTS idx_family_member_locations_family
    ON public.family_member_locations(family_id, user_id);

CREATE INDEX IF NOT EXISTS idx_family_member_locations_updated
    ON public.family_member_locations(updated_at DESC);

ALTER TABLE public.family_member_locations ENABLE ROW LEVEL SECURITY;

-- Active family members can read locations of other active members in same family
...
-- Users can only insert/update their own location row
